...
world.printRooms()
player = Player("Inaki", world.startingRoom)
...
def traverseGraph():
    ...
    while len(visited) < len(world.rooms):
        ...
        if player.currentRoom.n_to is not None:
            adjacentRooms['n'] = player.currentRoom.n_to.id
        else: 
            adjacentRooms['n'] = None
        if player.currentRoom.e_to is not None:
            adjacentRooms['e'] = player.currentRoom.e_to.id
        else: 
            adjacentRooms['e'] = None
        if player.currentRoom.s_to is not None:
            adjacentRooms['s'] = player.currentRoom.s_to.id
        else: 
            adjacentRooms['s'] = None
        if player.currentRoom.w_to is not None:
            adjacentRooms['w'] = player.currentRoom.w_to.id
        else: 
            adjacentRooms['w'] = None

        traversalGraph[player.currentRoom.id] = adjacentRooms

        if player.currentRoom.n_to is not None and player.currentRoom.n_to not in visited:
            goBackCache.append('s')
            traversalPath.append('n')
            player.travel('n')
        elif player.currentRoom.e_to is not None and player.currentRoom.e_to not in visited:
            goBackCache.append('w')
            traversalPath.append('e')
            player.travel('e')
        elif player.currentRoom.s_to is not None and player.currentRoom.s_to not in visited:
            goBackCache.append('n')
            traversalPath.append('s')
            player.travel('s')
        elif player.currentRoom.w_to is not None and player.currentRoom.w_to not in visited:
            goBackCache.append('e')
            traversalPath.append('w')
            player.travel('w')
        elif len(visited) < len(roomGraph):
            while len(goBackCache) > 0:
                direction = goBackCache.pop()
                traversalPath.append(direction)
                player.travel(direction)

                if player.currentRoom.n_to is not None and player.currentRoom.n_to not in visited or player.currentRoom.e_to is not None and player.currentRoom.e_to not in visited or player.currentRoom.s_to is not None and player.currentRoom.s_to not in visited or player.currentRoom.w_to is not None and player.currentRoom.w_to not in visited:
                    break

            """ shortestPath = findShortestPath(player.currentRoom, [], visited)
    ...
    print(traversalPath)
    for i in sorted(traversalGraph.keys()) :  
        print(i, '  ', traversalGraph[i]) 

def findShortestPath(room, path, visited):
    ...
    if room.n_to is not None:
        newPath = path.copy()
        newPath.append('n')
        path_n = findShortestPath(room.n_to, newPath, visited)
    if room.e_to is not None:
        newPath = path.copy()
        newPath.append('e')
        path_e = findShortestPath(room.e_to, newPath, visited)
    if room.s_to is not None:
        newPath = path.copy()
        newPath.append('s')
        path_s = findShortestPath(room.s_to, newPath, visited)
    if room.w_to is not None:
        newPath = path.copy()
        newPath.append('w')
        path_w = findShortestPath(room.w_to, newPath, visited) 

    shortestPath = path_n
    if len(path_e) < len(shortestPath):
        shortestPath = path_e
    if len(path_s) < len(shortestPath):
        shortestPath = path_s
    if len(path_w) < len(shortestPath):
        shortestPath = path_w

    return path.extend(shortestPath)
